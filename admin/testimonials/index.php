...
$query = "SELECT * FROM testimonials ORDER BY is_featured DESC, created_at DESC";
      ...
      <table class="table mb-0 table-striped">
        <thead>
          <tr>
            ...
            <th>Текст</th>
            <th>Зураг</th>
            <th class="text-center">Онцлох</th>
            <th>Огноо</th>
            <th>Үйлдэл</th>
          </tr>
        </thead>
          ...
          <?php if($result && $result->num_rows > 0): ?>
            <?php while($row = $result->fetch_assoc()): ?>
              <tr>
                ...
                <td><?php echo htmlspecialchars(mb_substr(strip_tags($row['content']), 0, 80)); ?>...</td>
                <td>
                  <?php if($row['photo']): ?>
                    <img src="<?php echo UPLOAD_URL . $row['photo']; ?>" style="max-height:60px" alt="">
                  <?php endif; ?>
                </td>
                <td class="text-center"><?php echo $row['is_featured'] ? 'Тийм' : 'Үгүй'; ?></td>
            ...
            <?php endwhile; ?>
          <?php else: ?>
            <tr><td colspan="7" class="text-center py-4">Мэдээлэл олдсонгүй</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
